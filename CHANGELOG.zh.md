...
- 运行版本号收敛为编译期常量：`src/core/fun.pas` 新增 `funVersion`（数值构建戳，日期编码
  YYYYMMDD）与 `funRelease`（可读发布串，如 `'9.0'`）。脚本可用两条路径探测当前解释器是否够新，
  且二者读的是同一个常量、不会漂移：`N.time()`（N 在 2010..2099 时返回 `funVersion`）与
  `'host'.arg()` 新增的 `version` 字段。此前 `_time` 里的版本号是硬编码字面量，`'host'.arg()`
  也只有可读串 `fun`。回归 `lib-host-test.fun` 新增断言，验证两条路径相等。
...
- 把输入结尾视为隐式语句终结符：文件最后一条语句即使漏写结尾的 `;`，现在也会被解析并执行，
  不再被静默丢弃。此前词法层在输入结尾不产生任何 token，待归约的语句永远无法归约，脚本会以
  0 退出且没有任何输出（例如末行为 `?. x` 的程序什么都不打印）。改动只作用于输入结尾，
  语句之间漏写 `;` 仍然是语法错误。该逻辑位于词法运行时（`src/parse/lib/lexlib.pas`），
  不涉及任何生成的解析/词法表文件。
...
- 为容器新增键探测：`s.@has(key)` 在列表或集合确实含有该键时为真，即使其值是 nil。
  `m[key]` 无法区分键不存在与值为 nil（两者都读作 nil），而 `key in m` 扫描的是值而非键，

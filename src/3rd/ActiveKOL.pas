...
uses
  windows, messages, KOL, ActiveX, ComObj;
...
begin
  ...
  raise EOleError.CreateResFmt(Integer( @SBadPropValue ), [S]);
end;
...
var
  ...
  begin
    if Status = CLASS_E_NOTLICENSED then
      raise EOleError.CreateFmt(Ident, [SubClassName]);
    OleCheck(Status);
  end;
  ...
  try
    ...
    begin
      ...
      if FOleInPlaceObject = nil then
        raise EOleError.CreateResFmt(Integer( @SCannotActivate ), [nil]);
      HookControlWndProc;
...
begin
  ...
  FOleObject := nil;
  FEventDispatch := nil; //FEventDispatch.Free;  // zwd@funlang.org
  FreeList(FFonts);
  FreeList(FPictures);
  Dec(FControlData^.InstanceCount);
  if FControlData^.InstanceCount = 0 then
    DestroyEnumPropDescs;
  fOleCtlIntf := nil;    //fOleCtlIntf.Free;     // zwd@funlang.org
  inherited Destroy;
end;
...
begin
  GetProperty(Index, Temp);
  //Result := IUnknown(Temp.VUnknown); ***** Change to:
...
begin
  ...
  begin
    ...
    if WndHandle = 0 then
      raise EOleError.CreateResFmt(Integer(@SNoWindowHandle), [nil]);
    fHandle := WndHandle;
...
begin
  InterfaceConnect(Obj, FServerData^.EventIID, FEventDispatch, FEventsConnection);
end;

procedure TOleServer.DisconnectEvents(const Obj: Iunknown);
begin
  InterfaceDisconnect(Obj, FServerData^.EventIID, FEventsConnection);
end;
